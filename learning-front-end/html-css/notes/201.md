...
* Flow-overlap/collapse: A floated element's unfloated parent elements in a container will overlap the dimenstions of the floated element. The collapse is because the height of the parent container was being determined by the height of containing elements. Once these are removed the height will be zero*
  * *Technically, if the parent container has a fixed height, then the container won't collapse (it's not relying on the contained elements). 
...
The cleared element will also allow the parent elements to configure to the dimenstions of the original floated element.
...
* Height and width expand automatically, unless you set either of those properities with values
...
This is also reversible (you switch the left hand column to a fixed width) without having to change the HTML structure.
...
* Width: In general, avoid using fixed widths. Instead use vertical and horizontal padding to create the button shape you want. The base size will be determined by the inline contents (text).
...
2. `seperate` (default) - border widths will stack up
3. `inherit` 

However, one interesting effect you can make use of if opt for the default value `border-collapse: seperate` is that you can use `border-spacing`

> `border-spacing` - determines space between borders. Only usable with `border-collapse: seperate`
...
* Borders: Adding a `border-radius` creates an inset border, so specify a border as well.
...
The basics for seeting up a background image
...
If using a psuedo-selector make sure to include:
